...
function scaleToFillComp() {
  ...
  if (checkComp(activeComp)) {
    ...
    if (comp.selectedLayers.length > 0) {
      ...
      var propertiesToAdd = {
        "Anchor Point":
          "R = thisLayer.sourceRectAtTime(); T = R.top; L = R.left; W = R.width; H = R.height; [L+W/2,T+H/2]",
        Position: "[thisComp.width/2, thisComp.height/2]",
        Scale:
          "p = position; xScale = p[0] > thisComp.width/2 ? p[0]/(width/2) : (thisComp.width - p[0])/(width/2); yScale = p[1] > thisComp.height/2 ? p[1]/(height/2) : (thisComp.height - p[1])/(height/2); s = Math.max(xScale,yScale); [s,s]*100",
      };
...
function CreateParentNull() {
  ...
  try {
    ...
    for (
      var layer_index = 0;
      layer_index < selectedLayers.length;
      layer_index++
    ) {
  ...
  if (compIndex) {
    ...
    function askForName() {
      ...
      if (name) {
        ...
        showAlertWindow("No input provided. Please enter a valid name.");
      }
    }
...
btn_createIMGComps.onClick = function () {
  var isBoiler = findCompIndex("BPLATE",true);
  var compIndex = findCompIndex("__SETTINGS");
...
changeProjectName.onClick = function () {
  ...
  if (!compIndex) {
    showAlertWindow(
      "You need to create a Template before you can use this feature."
    );
  } else {
