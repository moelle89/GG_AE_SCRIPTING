...
(function (thisObj) {
	...
	***************************************************************************/
	...
	var ddl_presets = content.add('dropdownlist', undefined, iconList); // presets dropdown, populated with the array of all elemnts in icons objec
	ddl_presets.selection = 2; // default to the first preset index

	var et_coords = content.add('edittext', undefined, getCoordString(ddl_presets.selection.text), {multiline: 'true'}); // coordinates edittext box
		et_coords.minimumSize.height = 90;
		et_coords.maximumSize.height = 90;

	var grp_buttons = content.add('group'); // group to store lower button row
		grp_buttons.margins = [0, 4, 0, 10];

	var btn_update = buttonColorText(grp_buttons, 'Refresh', '#546E7A', '#55869d'); // refresh button
		btn_update.alignment = ['fill', 'fill'];

	var cp = grp_buttons.add('edittext', undefined, '#00BCD4'); // hex color edittext
		cp.alignment = ['right', ''];
		cp.maximumSize.width = 72;

	var btn_about = grp_buttons.add('button',undefined,'?'); // launch about panel
	...
	function updateButton(pGrp) { // draw UI elements
		if (pGrp.children.length > 0) {
			pGrp.remove(pGrp.children[pGrp.children.length-1]); // remove the last element in group
		}

		var goodQuotes = et_coords.text // replace quotes with dumbquotes
			.replace(/[\u2018\u2019]/g, "'")
			.replace(/[\u201C\u201D]/g, '"');

		var btn_reference = buttonColorVector(pGrp, JSON.parse(goodQuotes), [60, 24], cp.text, '#baeff6'); // draw button with new values
	...
	ddl_presets.onChange = function() { // preset is changed, update UI
		et_coords.text = getCoordString(ddl_presets.selection.text);
		updateButton(grp_output);
	};

	et_coords.onChange = function() { // if edittext box changed, reset dropdownlist to  'Select a preset'
		ddl_presets.selection = 0;
	};

	btn_update.onClick = function() { // click UPDATE and fresh UI elements
		try { // test if coords work
			updateButton(grp_output);
		} catch(e) {
			alert('Oops. Got some bad coordinates there. Try again.');
			return;
		}
	};
	...
	btn_about.onClick = function() { // open about panel
		...
		var btn_url = buttonColorVector(content, headerVec, [192,48], '#FFA726', '#ffb74d');
