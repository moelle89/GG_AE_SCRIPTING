...
REM aerender task for comp using -RStemplate "getgenius_preview" -OMtemplate "getgenius_jpg_preview" to export 8 jpg frames evenly distributed over the whole duration of the composition
    "C:\Program Files\Adobe\Adobe After Effects 2024\Support Files\aerender.exe" -project "%Folder%\!inputFile!" -comp "!compName!" -v ERRORS -mp -RStemplate "getgenius_preview" -OMtemplate "getgenius_jpg_preview" -output "%RenderedFolder%\[compName]_[###].jpg"

REM convert the exported frames into a .webm video using ffmpeg
	"C:\ffmpeg\bin\ffmpeg.exe" -framerate 3 -start_number 0 -i "%RenderedFolder%\!compName!_%%03d.jpg" -c:v libvpx-vp9 -quality good -speed 14 -crf 23 -c:a libopus "%Folder%\!compName!.webm"
